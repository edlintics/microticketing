...
global.signin = (id?: string) => {
  // for saving user information when sign in, we provide an optional argument of id
  // Build a JWT payload. {id, email, }
  const payload = {
    id: id || new mongoose.Types.ObjectId().toHexString(), // if id was provided in the parameter, we use that id, if not create a random new one
    email: "test@test.com",
  };
